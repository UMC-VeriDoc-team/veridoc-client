...
const UsageGuideDetailContent = () => {
  ...
  const content = USAGE_GUIDE_DETAIL_CONTENTS[guideType];

  // 현재 guideType(id)과 일치하는 카드를 찾아 cardNumber 알아내기
  const currentCard = USAGE_GUIDE_CARDS.find((card) => card.id === guideType);
...
export default UsageGuideDetailContent;
